> ⚠️ **ARCHIVED / SUPERSEDED (04-07-2026).** This document describes an
> earlier, **headless data-processing pipeline** product ("no graphical UI")
> that does **not** match the product actually built. The shipped system is an
> interactive web app (Next.js frontend + FastAPI backend + Celery pipeline
> with a monitoring UI). For the current, authoritative product spec see
> [`csv-agent-services-prd.md`](./csv-agent-services-prd.md). Kept only for
> historical reference — do **not** use it to describe current scope.
...
├── data/data_anonymized/         # Benchmark datasets (CICIDS2017, Credit Card Fraud, etc.)
├── notebooks/
